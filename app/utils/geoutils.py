...
neighborhood_types = ['neighborhood', 'political']
...
class LocationData():
    ...
    def parse_raw(data):
        ...
        if not city:
            city = [comp.get("long_name") for comp in rawgoogledata if comp.get("types") == neighborhood_types]
        if not city:
    ...
    def build(geocoder_results):
        data = LocationData.parse_raw(geocoder_results.raw)
        return LocationData(
    ...
    def from_string(blob):
        ...
        return LocationData.build(results)
    ...
    def from_structured(dct):
        ...
        data = LocationData.parse_raw(results.raw)
        return LocationData.build(results)
...
# test = LocationData.from_string(test_str)
# print(test.__dict__)
# print(test.address)

# test = LocationData.from_string(test2_str)
# print(test.__dict__)

# test = LocationData.from_structured(test2_dict)
# print(test.__dict__)

# test = LocationData.reverse(test3_lat, test3_lon)
# print(test.__dict__)

# test = LocationData.reverse(35.7099552,139.8104001)
# print(test.__dict__)
