...
function CommonNameHeader({taxon}) {
    ...
    return taxon.common_name ? <span className="display-name-header">{common}</span> : <span className="sci-name-header">{scientific}</span>;
}
...
function ScientificNameHeader({taxon}) {
    ...
    return <span className="sci-name-subheader">({rank} {scientific})</span>
}
